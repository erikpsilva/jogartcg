...
try {
 ...
 if($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  $values=[$name,$type,$image,$description,$prices['gold'],$prices['reais'],isset($_POST['active'])?1:0];
  if($id===''){$id='product-'.bin2hex(random_bytes(12));$q=$pdo->prepare('INSERT INTO shop_products (name,type,image,description,gold,reais,active,id) VALUES (?,?,?,?,?,?,?,?)');}
  else $q=$pdo->prepare('UPDATE shop_products SET name=?,type=?,image=?,description=?,gold=?,reais=?,active=? WHERE id=?');
