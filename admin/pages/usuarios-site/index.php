...
<div class="adminLayout"><?php include ROOT . '/admin/includes/sidebar/sidebar.php'; ?><main class="adminLayout__content siteUsers">
    ...
    <?php if ($problem): ?><div class="formAlert formAlert--error" role="alert"><?= suEscape($problem) ?> <?php if ($id && $user): ?><a href="<?= ADMIN_BASE_URL ?>/usuarios-site?id=<?= $id ?>">Reabrir perfil</a><?php endif; ?></div><?php endif; ?>
    <?php if ($listing): ?>
    ...
    <form method="get" class="siteUsers__filters formGroup" action="<?= ADMIN_BASE_URL ?>/usuarios-site"><div class="formGroup__item siteUsers__search"><label for="su-search">Buscar jogador</label><input id="su-search" type="search" name="q" value="<?= suEscape($listing['q']) ?>" placeholder="Nome, e-mail ou CPF" maxlength="190"></div><div class="formGroup__item"><label for="su-status-filter">Status</label><select id="su-status-filter" name="status"><option value="">Todos</option value="ativo" <?= $listing['status']==='ativo'?'selected':'' ?>>Ativos</option><option value="inativo" <?= $listing['status']==='inativo'?'selected':'' ?>>Inativos</option></select></div><div class="formGroup__item"><label for="su-beta-filter">Acesso beta</label><select id="su-beta-filter" name="beta"><option value="">Todos</option><option value="1" <?= $listing['beta']==='1'?'selected':'' ?>>Beta testers</option><option value="0" <?= $listing['beta']==='0'?'selected':'' ?>>Sem acesso beta</option></select></div><button class="btn btn--primary" type="submit">Filtrar</button><a class="siteUsers__clear" href="<?= ADMIN_BASE_URL ?>/usuarios-site">Limpar</a></form>
    ...
    <a class="siteUsers__back" href="<?= ADMIN_BASE_URL ?>/usuarios-site">← Voltar para usuários</a>
    ...
    <?php if ($deleteView): ?>
    <form class="siteUserDanger formGroup" method="post" action="<?= ADMIN_BASE_URL ?>/usuarios-site?id=<?= $id ?>&amp;acao=excluir" novalidate data-site-user-form>
        ...
        <div class="siteUserActions"><a class="btn btn--gray" href="<?= ADMIN_BASE_URL ?>/usuarios-site?id=<?= $id ?>">Cancelar</a><button type="submit" class="btn btn--danger">Excluir conta e decks</button></div>
    </form>
    <?php else: ?>
    <form class="formGroup siteUserForm" method="post" enctype="multipart/form-data" action="<?= ADMIN_BASE_URL ?>/usuarios-site?id=<?= $id ?>" novalidate data-site-user-form>
        ...
        <div class="siteUserActions"><a class="btn btn--gray" href="<?= ADMIN_BASE_URL ?>/usuarios-site">Voltar à lista</a><button class="btn btn--primary" type="submit">Salvar alterações</button></div>
    </form><section class="siteUserDanger adminCard"><h2>Excluir conta</h2><p>A exclusão é permanente e também remove os decks deste jogador. Você verá uma confirmação antes de continuar.</p><a class="btn btn--danger" href="<?= ADMIN_BASE_URL ?>/usuarios-site?id=<?= $id ?>&amp;acao=excluir">Excluir usuário</a></section>
    <?php endif; ?>
    <?php elseif (!$listing): ?><a class="btn btn--gray" href="<?= ADMIN_BASE_URL ?>/usuarios-site">Voltar para usuários</a><?php endif; ?>
