...
export function RewardShortcuts({ assetBase = './' }: { assetBase?: string }) {
  const [selected, setSelected] = useState<'daily' | 'pass' | null>(null);
  ...
  return <div className="reward-shortcuts">
    ...
    {selected && createPortal(<dialog ref={dialog} className={`reward-modal reward-modal--${selected}`} aria-labelledby="reward-modal-title" onCancel={() => setSelected(null)} onClose={() => setSelected(null)} onClick={(event) => {
      ...
      <header className="reward-modal-heading"><img src={`${assetBase}${selected === 'daily' ? 'adventure/mickey-idle.gif' : 'shop/season-pass-icon-v1.png'}`} alt="" /><div><small>{selected === 'daily' ? 'UM PRESENTE A CADA NOVO DIA' : '30 DIAS DE CONQUISTAS'}</small><h2 id="reward-modal-title">{selected === 'daily' ? 'Login de 7 dias' : 'Passe de temporada'}</h2></div><button type="button" autoFocus aria-label="Fechar" onClick={() => setSelected(null)}>×</button></header>
      <RewardsPage key={selected} initialView={selected} compact assetBase={assetBase} />
