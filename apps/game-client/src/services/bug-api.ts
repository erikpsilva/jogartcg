...
export async function getBugChallenge(): Promise<string> {
  const response = await fetch(`${API_BASE_URL}/bugs/challenge`, { credentials: 'include' });
...
export async function sendBugReport(report: string, captcha: string, csrfToken: string): Promise<string> {
  const response = await fetch(`${API_BASE_URL}/bugs`, { method: 'POST', credentials: 'include', headers: { 'Content-Type': 'application/json', 'X-CSRF-Token': csrfToken }, body: JSON.stringify({ report, captcha, page: window.location.href }) });
  const result = await body(response);
